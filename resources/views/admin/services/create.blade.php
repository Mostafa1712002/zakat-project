...
<div class="page-header">
    <div>
        <h1>🛠️ إضافة خدمة</h1>
        <p>الخدمة لا تحمل سعراً ثابتاً — يُدخل السعر مع كل فاتورة</p>
    </div>
</div>
...
<form method="POST" action="{{ route('admin.services.store') }}" class="card" style="padding:1.5rem;max-width:720px">
    @csrf
    ...
    <div class="form-group">
        ...
        <select id="default_zatca_classification" name="default_zatca_classification" class="form-control" required>
            <option value="S" @selected(old('default_zatca_classification', 'S') === 'S')>S — قياسية (15%)</option>
            <option value="Z" @selected(old('default_zatca_classification') === 'Z')>Z — صفرية</option>
            <option value="E" @selected(old('default_zatca_classification') === 'E')>E — معفاة</option>
        </select>
    </div>
