...
@section('title', 'تسجيل معاملة شريك')

@section('content')
<div class="page-header">
    <div>
        <h1>➕ تسجيل معاملة جديدة</h1>
        <p>تسجيل سحب أرباح أو توزيع أو استثمار</p>
    </div>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('partner-transactions.store') }}" method="POST">
            @csrf
            ...
            <div class="form-row">
                    ...
                    <select name="partner_id" id="partner_id" class="form-control" required>
                        ...
                        @foreach($partners as $partner)
                            <option value="{{ $partner->id }}"
                                data-percentage="{{ $partner->ownership_percentage }}"
                                {{ old('partner_id', $selectedPartner) == $partner->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="type" id="type" class="form-control" required>
                        <option value="withdrawal" {{ old('type', $selectedType) == 'withdrawal' ? 'selected' : '' }}>💸 سحب أرباح</option>
                        <option value="profit_share" {{ old('type', $selectedType) == 'profit_share' ? 'selected' : '' }}>📊 توزيع أرباح</option>
                        <option value="investment" {{ old('type', $selectedType) == 'investment' ? 'selected' : '' }}>💵 إضافة رأس مال</option>
                        <option value="return" {{ old('type', $selectedType) == 'return' ? 'selected' : '' }}>↩️ إرجاع رأس مال</option>
                    </select>
                    @error('type')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="period" id="period" class="form-control" value="{{ old('period') }}" placeholder="مثال: 2026-Q1 أو 2026">
                    <small class="form-help">الفترة التي تتعلق بها المعاملة (ربع سنوي أو سنوي)</small>
                    @error('period')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                </div>
