...
<div class="page-header">
    <div>
        <h1>✏️ تعديل تسعيرة مبيعات</h1>
        <p>{{ $sale->invoice_number }}</p>
    </div>
    <div class="header-actions">
        <a href="{{ route('quotations.show', $sale) }}" class="btn">عرض</a>
...
<form action="{{ route('quotations.update', $sale) }}" method="POST" id="quotationForm">
    @csrf
    @method('PUT')
    ...
    <div class="card" style="margin-top: 1.5rem;">
        <div class="card-body">
            ...
            <a href="{{ route('quotations.show', $sale) }}" class="btn btn-lg">إلغاء</a>
        </div>
    </div>
</form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const productsData = @json($products->mapWithKeys(fn($p) => [$p->id => ['retail' => $p->selling_price, 'area_per_unit' => (float) ($p->area_per_unit ?? 0)]]));
    ...
    function updateRowPrice(row, skipPriceUpdate = false) {
        ...
        if (!product) { if (!skipPriceUpdate) priceInput.value = 0; calculateTotals(); return; }
        if (!skipPriceUpdate) priceInput.value = product.retail;
        calculateTotals();
    }

    function attachRowEvents(row) {
        row.querySelector('.product-select').addEventListener('change', function() { updateRowPrice(row); });
